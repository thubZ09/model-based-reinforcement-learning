"""
Paper -"Deep Reinforcement Learning in a Handful of Trials using Probabilistic Dynamics Models"
Authors: Chua et al. (2018)
Link: https://arxiv.org/abs/1805.12114

Overview
>Collect data from environment
>Train ensemble of probabilistic dynamics models
>Plan actions using CEM (Cross-Entropy Method)
>Execute best action
>Repeat
"""
...
class EnsembleDynamics(nn.Module):    
    ...
    def _build_network(self, state_dim: int, action_dim: int, hidden_size: int) -> nn.Module:
        return nn.Sequential(
            ...
            nn.Linear(hidden_size, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, state_dim + 1)  # +1 for reward
        )
        ...
        for _ in range(config.max_episode_steps):
            with torch.no_grad():
                action = cem_planning_continuous(
                    ...
                    action_dim=1,  # Pendulum has 1D action
